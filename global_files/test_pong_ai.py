...
import argparse
import wimblepong
from simple_dqn.simple_dqn_agent import Agent as SimpleDqnAgent
import hps
import torch
...
player = SimpleDqnAgent()#wimblepong.SimpleAi(env, player_id)
...
if not args.test:
    print("Training")
    #Test network saving
    torch.save(player.policy_net.state_dict(), "../local_files/trained_nets/dq_net_at_start.pth")

    while player.frames_seen < TARGET_FRAMES:
        done = False
        games += 1
    ...
    torch.save(player.policy_net.state_dict(), "../local_files/trained_nets/dq_net_at_end.pth")
else:
    print("Testing")
    player.load_model("../local_files/trained_nets/net_at_70174_games.pth")
    for games in range(100):
        ...
        while not done:
            steps += 1
            action = player.get_action(observation, False)
            observation, reward, done, info = env.step(action)
